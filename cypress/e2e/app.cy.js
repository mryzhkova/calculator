...
describe('Checks operations', () => {
  it('check sum operator', () => {
    ...
    cy.get(
      `[data-cy=${dataCyValues.currentOutput}]`,
    ).should('have.text', '8');
    cy.get('[data-cy="+"]').click();
    cy.get(
      `[data-cy=${dataCyValues.currentOutput}]`,
    ).should('have.text', '+');
    cy.get(`[data-cy=${dataCyValues.output}]`).should(
      'have.text',
      '8+',
    );
    ...
    cy.get('[data-cy="0h"]').should('have.text', '8 + 2');
  });
  ...
  it('check sub operator', () => {
    ...
    cy.get('[data-cy="1h"]').should('have.text', '30 - 5');
  });
  ...
  it('checks mul operator', () => {
    ...
    cy.get('[data-cy="2h"]').should('have.text', '6 * 10');
  });
  ...
  it('checks div operator', () => {
    ...
    cy.get(
      `[data-cy=${dataCyValues.currentOutput}]`,
    ).should('have.text', '12.5');
    cy.get('[data-cy="3h"]').should('have.text', '75 / 6');
  });
  ...
  it('checks rest operator', () => {
    ...
    cy.get('[data-cy="4h"]').should('have.text', '368 % 7');
  });
  ...
  it('checks () operator', () => {
    ...
    cy.get('[data-cy="output"]').should('have.text', '2*');
    cy.get('[data-cy="8"]').click();
    cy.get('[data-cy="+"]').click();
